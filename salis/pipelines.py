...
class SalisPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('salis.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `salis`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM salis WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO salis (date, title, link, content)"
